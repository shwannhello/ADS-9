...
void PMTree::collectPermsSequential(std::shared_ptr<Node> node, std::vector<char>& current,
                                   std::vector<std::vector<char>>& result, int& counter, 
                                   int target, std::vector<char>& targetPerm, bool& found) {
    if (found) return;
    ...
    for (auto& child : node->children) {
        if (found) return;
        current.push_back(child->value);
        collectPermsSequential(child, current, result, counter, target, targetPerm, found);
        current.pop_back();
    }
}
...
std::vector<char> getPerm1(PMTree& tree, int num) {
    ...
    int totalPerms = 1;
    for (int i = 2; i <= tree.originalElements.size(); ++i) {
        totalPerms *= i;
    }
    ...
    tree.collectPermsSequential(tree.root, current, result, counter, num, result, found);
    
    return result;
}

void PMTree::findPermByPath(std::shared_ptr<Node> node, std::vector<char>& current,
                           int& remaining, int target, std::vector<char>& result, bool& found) {
    if (found) return;
    ...
    if (childIndex < node->children.size()) {
        ...
        remaining--;
        findPermByPath(node->children[childIndex], current, remaining, newTarget, result, found);
    }
}
...
std::vector<char> getPerm2(PMTree& tree, int num) {
    ...
    std::vector<char> current;
    int remaining = tree.originalElements.size();
    bool found = false;
...
int PMTree::getTotalPermsCount() {
    int count = 1;
    for (int i = 2; i <= originalElements.size(); ++i) {
        count *= i;
    }
    return count;
}
